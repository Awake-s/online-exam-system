...
<script setup lang="ts">
  ...
  // notice 指向 <ArtNotification> 组件实例，组件内 defineExpose({ unreadTotal })
  // 标注最小结构类型避免 TS 将 ref(null) 推断为 Ref<null> 导致 notice?.unreadTotal 报 never
  const notice = ref<{ unreadTotal: number } | null>(null)
