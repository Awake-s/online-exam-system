...
/**
 * 获取用户信息
 * 将考试系统后端返回的用户数据转换为 art-design-pro 格式
 */
async function fetchUserInfo(): Promise<void> {
  const userStore = useUserStore()
  const raw: any = await fetchGetUserInfo()
  ...
  // 如果后端返回的已经是转换后的格式（登录时已设置），直接使用
  if (raw.roles && Array.isArray(raw.roles)) {
    userStore.setUserInfo(raw)
  } else {
    // 将后端原始格式转换为 art-design-pro 格式
    const roleCode = raw.roleCode || 'STUDENT'
